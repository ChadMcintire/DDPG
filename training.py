...
def training_loop(args):
    ...
    replay_buffer = ReplayBuffer(state_dim, action_dim) # Step 2 Pseudocode
    ...
    for t in range(int(args.max_timesteps)): 
        ...
        if t < args.start_timesteps: 
            action = env.action_space.sample() # Step 3.i Pseudocode 
        else:
            action = (
                        policy.select_action(np.array(state))
                        + np.random.normal(0, max_action * args.expl_noise, size=action_dim) 
                     ).clip(-max_action, max_action) # Step 3.i Pseudocode 

        # Perform action  
        next_state, reward, done, _ = env.step(action)  # Step 3.ii and 3.iii Pseudocode 
        done_bool = float(done) if episode_timesteps < env._max_episode_steps else 0 # Step 3.iii Pseudocode 

        #store data in replay buffer
        replay_buffer.add(state, action, next_state, reward, done_bool) # Step 3.iv Pseudocode 
        ...
        if t >= args.start_timesteps:
            policy.train(replay_buffer, args.batch_size) #Step 3.vi.a Pseudocode
        ...
        if done:
            ...
            #Reset environment
            state, done = env.reset(), False # Step 3.v Pseudocode 
            episode_reward += reward
            episode_timesteps = 0
            episode_num += 1
